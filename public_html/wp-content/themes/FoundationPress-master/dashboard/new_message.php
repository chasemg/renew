...
if($_POST['message_id'] != 0)	{
	...
	foreach($message as $m)	{
		...
		$html .= '<div>Reply date: '.date("M d, Y h:i:s A").'<input type="hidden" id="send_date" value="'.strtotime($current_date).'"></div>';
		$html .= '</div>';
		$html .= '<hr>';
	...
	$html .= '<div class="title">Subject: <input id="subject" type="text" value=""></div>';
	$html .= '<hr>';
	...
	$current_date = date("m/d/Y H:i:s");
	$html .= '<div>Send date: '.date("M d, Y").'<input type="hidden" id="send_date" value="'.strtotime($current_date).'"></div>';
	$html .= '</div>';
	$html .= '<hr>';
	$html .= '<div class="message_text">Message: <textarea id="message_text"></textarea></div>';	
}
