...
class TPC_CRM_Admin_FeaturePointers implements TPC_Interface_Runnable {
	...
	public function __construct( ) {
		$this->steps = array( 
			...
			array(
				...
				"message"  => sprintf( "<p>If you purchase the <a href=\"%s\">Premium version</a> you can hide or show additional columns including <a href=\"%s\">Custom Fields</a>.</p>",
							"http://www.theportlandcompany.com/product/customer-relationship-manager-plugin-for-wordpress",
							"http://www.advancedcustomfields.com/resources/getting-started/what-is-acf/" )
			),
			array(
				"pointer"  => ".wp-list-table th:nth-child(2)",
				"position" => "bottom",
		...
		$this->steps[] = array(
				...
				"position" => "right",
				"message"  => addslashes( "<p>You can use date strings (e.g. today, last week, last month) to query date columns. NOTE: The global search will only search for the visible columns. For numeric fields, it'll look for the exact value while it'll look for the month for date fields (e.g. if you put in July 10, 2014, it'll look for items that are on July)." )
			);
	}
